...
function changeLang(tthis) {
  ...
  switch (lang) {
    case "it-IT":
       window.open("/<?PHP echo($AVATAR_NAME);?>/blog/?hl=it-IT","_self");
       break;
    case "en-US":
       window.open("https://ottimo-ooo.translate.goog/<?PHP echo($AVATAR_NAME);?>/blog?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp","_self");
       break;
    case "zh-CN":
       //window.open("https://blog-gaoxio-com.translate.goog/<?PHP echo($AVATAR_NAME);?>?_x_tr_sl=auto&_x_tr_tl=zh&_x_tr_hl=en&_x_tr_pto=wapp","_self");
       window.open("http://webtrans.yodao.com/webTransPc/index.html?from=auto&to=zh-CHS&type=1&url=https%3A%2F%2Fottimo.ooo%2F<?PHP echo($AVATAR_NAME);?>%2Fblog", "_self");
       break;
   }    
...
/*
function changeLang(tthis) {
  window.open("/<?PHP echo($AVATAR_NAME);?>/blog/?hl="+$(tthis).val(),"_self");
}*/
...
$("div.dragover").on("drop", function(e) {
  ...
  if (count !== 0) {
    ...
    $.ajax("/<?PHP echo($AVATAR_NAME);?>/blog", {
        method: "POST",
        processData: false,
        contentType: false,
        data: fd//,
...
window.addEventListener("load", function() {
 ...
 <?PHP else: ?>
  ...
  for (i=1;i<=<?PHP echo($CUDOZ);?>;i++) {
    $("#cudozentry"+i).get(0).src="/<?PHP echo($AVATAR_NAME);?>/res/chicca_<?PHP echo($shortLang);?>.png";
  }  
 <?PHP endif; ?>
